...
try:
    ...
    if df.empty:
        ...
        for _, row in filtered_df.iterrows():
            # Sử dụng .get để tránh lỗi nếu thiếu cột
            ...
            st.markdown(f'''
                <div style="display: flex; gap: 5px; margin-top: 5px; margin-bottom: 20px;">
                    ...
                    <a href="{download_url if is_dl else '#'}" target="_blank" style="flex: 3; text-decoration:none;">
                        <button style="width:100%; height:38px; border-radius:5px; border:none; background-color:{'#b2d600' if is_dl else '#cbd695'}; color:white; cursor:{'pointer' if is_dl else 'not-allowed'};">
